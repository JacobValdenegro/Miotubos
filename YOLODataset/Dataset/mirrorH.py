...
output_dir = "./mirrorH"  # Carpeta de salida
...
for file_name in os.listdir(input_dir):
    if file_name.endswith(".json"):  # Filtrar solo archivos JSON
        ...
        # Leer la imagen para obtener el ancho
        image = cv2.imread(image_path)
        if image is None:
            print(f"Imagen no encontrada: {image_path}")
            continue

        img_width = image.shape[1]  # Ancho de la imagen

        # Crear espejo de la imagen
        mirrored_image = cv2.flip(image, 1)  # Flip horizontal

        # Ajustar las anotaciones
        for shape in data["shapes"]:
            original_points = shape["points"]
            mirrored_points = [[img_width - x, y] for x, y in original_points]  # Reflejar X
            shape["points"] = mirrored_points  # Actualizar los puntos en el JSON

        # Modificar el nombre de la imagen y del JSON en los datos
        mirrored_image_name = f"mirrored_{image_name}"
        mirrored_json_name = f"mirrored_{file_name}"
